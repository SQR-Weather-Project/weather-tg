...
async def set_frequency(msg: Message, state: FSMContext):
    ...
    await msg.answer("Please choose a parameter:", reply_markup=ReplyKeyboardMarkup(
        keyboard=[
...
async def set_parameter(msg: Message, state: FSMContext):
    ...
    await msg.answer("Please choose a filter:", reply_markup=ReplyKeyboardMarkup(
...
async def set_filter(msg: Message, state: FSMContext):
    ...
    await msg.answer(f"Notify if {(await state.get_data()).get('parameter')} is {msg.text}... (enter threshold value)")
...
async def set_city(msg: Message, state: FSMContext):
    ...
    await msg.answer(f"✅ Settings saved:\n"
...
async def get_user_settings(msg: Message, state: FSMContext):
    data = await state.get_data()
    if not data:
        await msg.answer("⚠️ You don't have any saved settings yet. Use /settings to set them up.")
        return

    await msg.answer(f"🔔 Your notification settings:\n"
